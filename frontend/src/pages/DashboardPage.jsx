...
const BudgetProgressBar = ({ spent, budget, percent, isLoading }) => {
  ...
  return (
    <article className="card">
      ...
      <div className="w-full h-3 bg-slate-100 dark:bg-slate-700 rounded-full overflow-hidden" role="progressbar" aria-valuenow={displayPercent} aria-valuemin={0} aria-valuemax={100}>
        <div className={`h-full rounded-full progress-bar-fill ${barColor}`} style={{ '--progress-width': `${displayPercent}%` }} />
      </div>
      {!isLoading && (
...
const DashboardPage = () => {
  ...
  // ── Fetch summary (✦ Fixed to Current Month) ─────────────────────────────
  ...
  return (
    <>
      {shouldShow && (
        <OnboardingModal onComplete={() => { markComplete(); setRefreshKey((prev) => prev + 1); }} />
      )}
      ...
      <header className="mb-6 flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Here's your financial snapshot for this month.</p>
        </div>
        <button onClick={() => setRefreshKey((prev) => prev + 1)} disabled={summaryLoading} className="btn-ghost mt-1 flex-shrink-0">
          <RefreshCw size={16} className={summaryLoading ? 'animate-spin text-emerald-500' : 'text-slate-400'} />
        </button>
      </header>

      {summaryError && <div className="alert-danger mb-4" role="alert">{summaryError}</div>}
      ...
      <section aria-label="Monthly budget progress" className="mb-6">
        <BudgetProgressBar spent={summary?.totalExpense} budget={summary?.monthlyBudget} percent={summary?.budgetUsedPercent} isLoading={summaryLoading} />
      </section>
      ...
      <section aria-label="Financial summary" className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        ...
        <StatCard label="Current Balance" value={summary?.balance} icon={Wallet} color={summary?.balance >= 0 ? 'blue' : 'rose'} isLoading={summaryLoading} />
      </section>
